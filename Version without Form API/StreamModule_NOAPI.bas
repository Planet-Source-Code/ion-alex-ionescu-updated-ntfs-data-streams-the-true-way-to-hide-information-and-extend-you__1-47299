...
Dim StreamName As String, Streams As String, StreamSizes As Long
...
NTFSStreamWriter.StreamStatus.Panels(1).Text = File                     ' You can remove this for even faster speed, but it might look like it's frozen
...
Do
    ...
    If StreamName <> "::$DATA" And StreamName <> ":encryptable:$DATA" Then                                      ' Add the stream to our stream list, except if it's the default one or encryption, also take off : and :$DATA
        Streams = Streams & Mid$(StreamName, 2, Len(StreamName) - 7) & ","                                      ' Add the stream name
        StreamSizes = StreamSizes + StreamInfo.StreamSize                                                       ' Add the stream size
    End If
...
If LenB(Streams) <> 0 Then                                              ' Check if file has streams
    Set ListObj = NTFSStreamWriter.ListView1.ListItems.Add(, , File)    ' Add the File
    ListObj.SubItems(1) = Streams                                       ' Add the streams
    ListObj.SubItems(2) = CStr(StreamSizes)                             ' Add the streams' size
End If
Exit Sub
End Sub
...
Handle = CreateFileW(StrPtr(File), AccessStandardRightsAll, ShareReadWrite, 0&, OpenExisting, BackupSemantics Or SequentialScan, 0&)
...
Do While cbRead
    ...
    With W32_STREAM_ID
        If .dwStreamNameSize Then                                                                                                  ' this is a named stream
            ...
            Set ListObj = NTFSStreamWriter.ListView1.ListItems.Add(, , Left$(Left$(StreamName, .dwStreamNameSize / 2), .dwStreamNameSize / 2 - 6)) 'format it
            ListObj.SubItems(1) = .Size & " bytes"                                                                                  ' add to listview (size)
            ListObj.SubItems(2) = StreamIDToString(.dwStreamId)                                                                     ' add to listview (id)
            ListObj.SubItems(3) = StreamAttributeToString(.dwStreamAttributes)                                                      ' add to listview (attribs)
        End If
        cbToSeek = W32_STREAM_ID.Size                                                                                               ' where to seek next
